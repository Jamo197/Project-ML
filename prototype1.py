...
def visualize_masks(
    ...
    for i in range(batch_size):
        ...
        # Upsample the context mask to match the image resolution
        context_mask_upsampled = F.interpolate(
            context_masks[i].unsqueeze(0).unsqueeze(0), size=(224, 224), mode="nearest"
        ).squeeze()

        # Apply the mask to the image
        masked_image = images[i].clone()
        masked_image[:, context_mask_upsampled == 1] = 0

        # Masked Image Visualization
        axes[1].imshow(masked_image.permute(1, 2, 0).cpu())
        ...
        for j in range(target_masks.shape[1]):
            target_mask_upsampled = F.interpolate(
                target_masks[i, j].unsqueeze(0).unsqueeze(0),
                size=(224, 224),
                mode="nearest",
            ).squeeze()
            masked_image = images[i].clone()
            masked_image[:, target_mask_upsampled == 1] = 0

            axes[j + 2].imshow(masked_image.permute(1, 2, 0).cpu(), cmap="gray")
...
def train(
    context_encoder, target_encoder, predictor, dataloader, epochs, save_path, device
):
        ...
        for batch_idx, (images, _) in enumerate(dataloader):
            ...
            for t in range(target_masks.size(1)):  # Iterate over target blocks
                ...
                mask_indices = torch.nonzero(
                    target_masks[:, t].flatten(1), as_tuple=False
                )[:, 1].unsqueeze(0)
                ...
                target_features.append(
                    full_target_embeddings[:, mask_indices.squeeze(1), :]
                )
...
if __name__ == "__main__":
    ...
    dataset_path = (
        r"C:\Users\jlamp\workspace\1_Projects\PML\data\archive\imagenet-mini\train"
    )
